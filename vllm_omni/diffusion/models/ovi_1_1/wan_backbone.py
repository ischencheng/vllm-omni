...
#
# The WanModel transformer below is adapted from vllm-omni's dreamid_omni
# integration, which itself ports the WAN 2.2 backbone for use in a
# twin-backbone fusion pipeline. The auxiliary classes and functions in the
# block below (RoPE helpers, modulation, norm, FFN, output head) are vendored
# from character-ai/Ovi (ovi/modules/model.py), Apache-2.0 licensed, so this
# file does not depend on the external `dreamid_omni` PyPI package.
...
class ConvMLP(nn.Module):
    """SwiGLU FFN built on Conv1d (used by the audio backbone)."""
