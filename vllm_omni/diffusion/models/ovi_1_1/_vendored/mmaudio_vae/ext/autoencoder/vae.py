...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # NOTE: upstream MMAudio uses ``nn.Buffer`` which is a PyTorch 2.6+ API;
        # ``register_buffer`` is the portable equivalent and works on any
        # version the rest of vllm-omni supports.
        if data_dim == 80:
            data_mean = torch.tensor(DATA_MEAN_80D, dtype=torch.float32).view(1, -1, 1)
            data_std = torch.tensor(DATA_STD_80D, dtype=torch.float32).view(1, -1, 1)
        elif data_dim == 128:
            data_mean = torch.tensor(DATA_MEAN_128D, dtype=torch.float32).view(1, -1, 1)
            data_std = torch.tensor(DATA_STD_128D, dtype=torch.float32).view(1, -1, 1)
        else:
